...
import { getCategoryBySlug, getRelatedCategories } from "@/database/categoryData"
...
import { ArrowLeft, BookOpen, Clock, Flame, Mountain, Shuffle, Zap } from "lucide-react"
...
export default function CategoryDetailPage() {
  ...
  const difficultyOptions = [
    {
      ...
      questionCount: questions.filter((q) => q.difficulty === TestDifficultyEnum.EASY).length,
      duration: 20,
    },
    {
      ...
      questionCount: questions.filter((q) => q.difficulty === TestDifficultyEnum.MEDIUM).length,
      duration: 30,
    },
    {
      ...
      questionCount: questions.filter((q) => q.difficulty === TestDifficultyEnum.HARD).length,
      duration: 45,
    },
    {
  ...
  const handleStartTest = (difficulty: TestDifficultyEnum) => {
    const option = difficultyOptions.find((opt) => opt.difficulty === difficulty)
    if (!option) return

    const testConfig: TestConfigT = {
      id: `${category.slug}-${difficulty.toLowerCase()}`,
      title: `${category.name} - ${difficulty}`,
      description: `Test your ${category.name} skills with ${difficulty.toLowerCase()} level questions`,
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-grow">
              ...
              <p className="text-lg text-muted-foreground mb-4 max-w-3xl leading-relaxed">{category.description}</p>
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">Select a difficulty level to start your assessment</p>
        </div>
          ...
          {difficultyOptions.map((option) => {
            ...
            return (
              <Card
                ...
                onClick={() => !isDisabled && handleStartTest(option.difficulty)}
              >
                <div className="flex flex-col h-full">
                  ...
                  {/* Button */}
                  <Button className="w-full" disabled={isDisabled} variant={isDisabled ? "outline" : "default"}>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedCategories.map((relatedCat) => (
                <Link key={relatedCat.id} href={`/categories/${relatedCat.slug}`} className="group">
